...
function setPanelItems() {
	// 패널 폭은 300
	const dist = (panelSize / 2) / Math.tan(unitRadian / 2) + (panelSize * 0.5);
...
function rotateToMouse(e) {
  ...
  const rotateX = ((topY - centerY) / centerY) * -10;
  const rotateY = ((leftX - centerX) / centerX) * 10;
...
gallery.addEventListener('mouseenter', () => {
  bounds = gallery.getBoundingClientRect();
